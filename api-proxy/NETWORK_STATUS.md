...
错误引用缓存的 `usage_limit_reached`，并要求等待约 113 小时。
该等待时间不能作为充值后的实时额度判断。

13:38:38 已重启经路径、配置和端口归属核验的反代进程；守护任务在约 6.39 秒内
恢复本地 API。配置与认证文件的 SHA-256 均未变化，继续使用直连及原有冷却策略。

13:39 对 `gpt-5.6-sol`、`gpt-5.6-luna`、`gpt-6-astra` 分别执行一次
Chat Completions 和一次 Responses 请求，六次均返回 HTTP 200 和 `OK`，
返回模型名称与请求一致，耗时 1.726–2.462 秒。现有登录可用，无需重新授权。
本次未独立查询充值余额；结论以这六次真实上游请求为依据。

最新脱敏证据：`%USERPROFILE%\.cli-proxy-api\quota-recovery-2026-09-25.json`。
恢复前响应和进程重启记录分别保存在同目录的 `quota-preflight-2026-09-25.json`
及 `cooldown-reset-process-2026-09-25.json`。

## 2026-09-23 13:13 清除过期额度冷却

已核对反代与本机 Codex 使用的是用户指定的同一 Pro 账号。
用户额度恢复后，反代进程仍缓存着此前 `usage_limit_reached` 的模型冷却状态，
因此本地直接返回 `model_cooldown`，没有重新向上游确认可用性。
12:45 检查得到的 9 月 27 日重试时间来自该旧缓存，不能据此判断实时剩余额度。

13:12:43 已重启受守护任务管理的反代，约 8.7 秒后恢复本地 API，清除了
内存中的旧冷却状态。配置文件、认证文件及默认冷却机制保持原样，继续直连。

13:13 对 Sol、Luna、Astra 分别执行 Chat Completions、Responses 请求，
六次均返回 HTTP 200 和 `OK`，模型名称一致，耗时 1.630–4.008 秒。
现有账号已可正常调用；上述 9 月 27 日等待时间不再适用。
本次验证没有独立查询额度百分比，因此不把用户提供的剩余百分比作为实测值。

最新脱敏证据：`%USERPROFILE%\.cli-proxy-api\quota-recovery-2026-09-23.json`。
早先的 `quota-check-2026-09-23.json` 已补充更正说明，保留当时响应供核对。
